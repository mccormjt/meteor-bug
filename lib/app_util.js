App = new function() {
    ...
    if (Meteor.isClient) {
        ...
        self.songQueue = function() {
            var songs = Songs.find({ _id: { $ne: App.cloud().nowPlayingSongId }, isQueued: true }).fetch();
            songs.sort(Songs.comparator);
            return songs;
        };

        self.anySongsQueued = function() {
            return Songs.find({ isQueued: true }, { fields: { _id: 1 } }).count() > 0;
        };
        ...
        self.getUserSongPriority = function() {
