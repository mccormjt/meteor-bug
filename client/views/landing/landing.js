...
function NearCloudsOptions() {
    ...
    function findAndSetupNearClouds() {
        ...
        $.when(getCurrentLocation())
            ...
            .then(function (data) {return Backend.getNearClouds(data.lat, data.long, data.accuracy)})
            .done(processNearCloudResults)
        ...
        function loadNearCloudResult(cloud) {
            var dist = parseFloat(cloud.distance).toFixed(2),
                result = '<div id=' + cloud.id + '><span>' + cloud.name + '</span><span>' + dist + ' mi.</span></div>';
    ...
    function enterCloudIdHandler() {
        ...
        if (fieldValue) {
            $.when(verifyCloudID(fieldValue))
                .done(function (data) {
                    loadAppWithParams(data.id, data.name, false);
                })
                .fail(function () {
                    $enterID_field.val("");
...
function CreateNewCloudOptions() {
    ...
    function createNewCloudHandler() {
        ...
        function setupNewCloud() {
            var isPublic = broadcastSliderButton.isOn ? 1 : 0;

            if (isPublic) {
                $.when(getCurrentLocation()).then(setup);
            }
            else {
                var data = {lat: "", long: "", accuracy: ""};
                setup(data);
            }

            function setup(data) {
                return $.when(Backend.postCreateCloud(getEscapedCloudName(), data.lat, data.long, data.accuracy, isPublic))
                    .done(function (data) {
                        loadAppWithParams(data.cloudID, data.name, true)
                    });
            }
        }
...
function loadAppWithParams(cloudID, name, isAdmin) {
    ...
    window.location.assign('../app.php?cloudID=' + cloudID + adminParam + '#' + name);
}

function getCurrentLocation() {
    var defer = $.Deferred(),
        data = {lat: "", long: "", accuracy: ""},
...
function verifyCloudID(id) {
    ...
    $.when(Backend.getCloud(id)).done(function (data) {
        if (data[0]) {
